...
p = np.array([6100., 3.42, 2.1, 0.31, 142.])
mod_fluxes = model.mod_fluxes(p)
...
for name in model.my_filters:
    ...
    ax[0].plot(wl, mod, "bo", ms=7, alpha=0.7)

    ax[1].plot(wl_scat, data - mod, "ro", ms=3.5, alpha=0.3)
    ax[1].errorbar(wl, 0, yerr=avg_err, ecolor="k")
